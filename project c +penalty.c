...
typedef struct {
    ...
    double y;     //znachenie celevoi func(mass+shtraf)
} Sample;
...
double objective(double x[4])
{
    ...
    //volume matirial
    double Vmat =
        (4.0 / 3.0) * PI * (pow(R + Th, 3) - pow(R, 3)) + PI * L * (pow(R + Ts, 2) - pow(R, 2));
    //massa
    ...
    if (sigma > sigma_allow)
        penalty += 1e7;
    return mass + penalty; //imenno eto optimiziryet algoritm
}
...
int main()
{
    ...
    {
        double best = dataset[0].y; //tekyshii min
        ...
        //вычисляем реальную функцию
        dataset[data_count].y = objective(best_candidate);
