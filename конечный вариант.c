...
typedef struct {
    ...
    double y;     //GP хранит только массу
} Sample;
...
int main()
{
    ...
    for(int i=0;i<INIT_POINTS;i++)
    {
        ...
        dataset[i].y=mass_function(dataset[i].x); //GP vidit tolko massu
    }
    ...
    for(int iter=0;iter<ITERATIONS;iter++)
    {
        ...
        //ищем текущую лучшую допустимую массу
        double best_mass = 1e12;
        for(int i=0;i<data_count;i++)
            if(penalty_function(dataset[i].x)==0 && dataset[i].y<best_mass)
                best_mass = dataset[i].y;

        double best_ei=-1;
        double best_candidate[4];

        //poisk sleduyushchey tochki s uchetom dopustimosti
        for(int c=0;c<CANDIDATES;c++)
        {
            double x[4];
            random_point(x);

            if(penalty_function(x) > 0) continue; //отбрасываем недопустимые

            double mu,sigma;
            gp_predict(x,&mu,&sigma);
            double ei=expected_improvement(mu,sigma,best_mass);
            if(ei>best_ei)
            {
                best_ei=ei;
        ...
        //добавляем новую точку (GP видит только массу)
        dataset[data_count].y = mass_function(best_candidate);
        for(int k=0;k<4;k++) dataset[data_count].x[k] = best_candidate[k];
        data_count++;

        printf("Iteration %d | Best mass = %.2f\n", iter+1,best_mass);
    }

    //итоговое решение
    int best_id=0;
    for(int i=1;i<data_count;i++)
        if(penalty_function(dataset[i].x)==0 && dataset[i].y<dataset[best_id].y)
            best_id=i;
